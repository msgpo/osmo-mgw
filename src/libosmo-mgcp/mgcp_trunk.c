...
int mgcp_trunk_equip(struct mgcp_trunk *trunk)
{
	...
	case MGCP_TRUNK_E1:
		...
		/* HACK! set up the E1 line early for the two endpoints we
		 * use for testing. */
		struct mgcp_endpoint *endp;
		endp = mgcp_endp_by_name(NULL, "ds/e1-1/s-2/su16-2@mgw", trunk->cfg);
		mgcp_e1_endp_equip(endp, 2, 4, 2);
		endp = mgcp_endp_by_name(NULL, "ds/e1-1/s-2/su16-4@mgw", trunk->cfg);
		mgcp_e1_endp_equip(endp, 2, 5, 4);
